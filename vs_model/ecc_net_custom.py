...
def block1_conv(x, custom_gradient=False):
    ...
    if custom_gradient:
        x = tf.keras.layers.Conv2D(64, (3, 3),
                        padding='same',
                        name='block1_conv2')(x)
        x = tf.keras.layers.Lambda(relu_with_custom_gradient)(x)
    else:
        x = tf.keras.layers.Conv2D(64, (3, 3),
                        activation='relu',
                        padding='same',
...
def block2_conv(x, custom_gradient=False):
...
def block3_conv(x, custom_gradient=False):
...
def block4_conv(x, custom_gradient=False):
...
def block5_conv(x, custom_gradient=False):
    x = tf.keras.layers.Conv2D(512, (3, 3),
                      activation='relu',
                      padding='same',
                      name='block5_conv1')(x)
...
def load_eccNET(vgg_model_path, stimuli_shape=(224, 224, 3), target_shape=(28, 28, 3), eccParam=eccParamDefault, ecc_depth=5, comp_layer='diff', customize_gradient=None):
    def VGG16(input_shape=(224, 224, 3), model_type=None):
        ...
        # Block 1 #############################################################
        block1_gradient = True if customize_gradient == 'block1' else False
        x = block1_conv(img_input, custom_gradient=block1_gradient)
        ...
        # Block 2 #############################################################
        block2_gradient = True if customize_gradient == 'block2' else False
        x = block2_conv(x, custom_gradient=block2_gradient)
        ...
        # Block 3 #############################################################
        block3_gradient = True if customize_gradient == 'block3' else False
        x = block3_conv(x, custom_gradient=block3_gradient)
        ...
        # Block 4 #############################################################
        block4_gradient = True if customize_gradient == 'block4' else False
        x = block4_conv(x, custom_gradient=block4_gradient)
        ...
        # Block 5 #############################################################
        block5_gradient = True if customize_gradient == 'block5' else False
        x = block5_conv(x, custom_gradient=block5_gradient)
