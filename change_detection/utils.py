...
def get_property(labels, meta_info, data_type="ecc"):
    property_array = np.array(
        [
            meta_info.loc[int(label.split("_")[0]), f"{data_type}_Left"]
            if label.split("_")[1] == "L"
            else meta_info.loc[int(label.split("_")[0]), f"{data_type}_Right"]
            for label in labels
        ]
    )
...
def plot_model_performance(x, y_baseline, y_eccNET, x_label, y_label, title=None):
    ...
    ax.plot(
        np.unique(x),
        m * np.unique(x) + b,
        label=f"baseline fit: r={np.corrcoef(x, y_baseline)[0,1]:.2f}",
    )
    ...
    ax.plot(
        np.unique(x),
        m * np.unique(x) + b,
        label=f"eccNET fit: r={np.corrcoef(x, y_eccNET)[0,1]:.2f}",
    )
...
def get_fixation_point(
    ...
    assert (
        fixation[0] > 0
        and fixation[0] < X_max
        and fixation[1] > 0
        and fixation[1] < Y_max
    ), "Fixation point out of image range."

    return tuple(np.array(fixation).astype(int))


def generate_and_preprocess_model_stim(stims, eye_res, preprocess=True, bg_color=0):
    ...
    temp_stim_size = (
        num_stims,
        stim_size[0] + 2 * eye_res,
        stim_size[1] + 2 * eye_res,
        num_channels,
    )
    temp_stims = np.ones(temp_stim_size) * bg_color
    temp_stims[
        :, eye_res : stim_size[0] + eye_res, eye_res : stim_size[1] + eye_res, :
    ] = np.copy(stims)
    if preprocess:
...
def downsample_attention_map(model_attention, attention_size):
    im = PIL.Image.fromarray(model_attention[0].astype(np.uint8)[:, :, 0])
    im = im.resize(attention_size, PIL.Image.ANTIALIAS)
